...
-spec merge([vnode_reply()]) -> {{appid}}_obj() | not_found.
merge(Replies) ->
    Objs = [Obj || {_,Obj} <- Replies],
    {{appid}}_obj:merge({{appid}}_entity_read_fsm, Objs).
...
%% @pure
different(A) -> fun(B) -> not {{appid}}_obj:equal(A,B) end.

%% @impure
%%
%% @doc Repair any vnodes that do not have the correct object.
-spec repair(atom(), string(), {{appid}}_obj(), [vnode_reply()]) -> io.
repair(_, _, _, []) -> io;

repair(VNode, StatName, MObj, [{IdxNode,Obj}|T]) ->
    case {{appid}}_obj:equal(MObj, Obj) of
        true -> repair(VNode, StatName, MObj, T);
        false ->
            {{appid}}_entity_vnode:repair(VNode, IdxNode, StatName, MObj),
